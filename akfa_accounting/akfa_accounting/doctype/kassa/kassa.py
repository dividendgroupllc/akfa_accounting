...
class Kassa(Document):
    ...
    def create_expense_journal_entry(self):
        ...
        if is_multicurrency:
            je.multi_currency = 1
            # validate_company_exchange_rates_available() buni validate() bosqichida
            # ushlab bo'lgan bo'lishi kerak -- bu yerda "1" bilan jim almashtirish
            # EMAS, aniq xato (himoyaning ikkinchi qatlami).
    ...
    def validate_company_exchange_rates_available(self):
        ...
        Frappe submit() oqimida docstatus=1 DB'ga on_submit() ishga tushishidan
        OLDIN yozilib bo'ladi. Agar kurs yo'qligi faqat on_submit() ichida
        aniqlansa, Kassa "submitted" bo'lib qoladi-yu, unga tegishli buxgalteriya
        yozuvi yaratilmay yoki muvozanatsiz qolib ketadi. Shuning uchun har bir
        on_submit() yo'li qaysi valyutalarga muhtoj bo'lsa -- shu yerda oldindan
        tekshiriladi."""
        ...
        if self.transaction_type == "Конвертация":
            ...
            # get_company_exchange_rate() chaqiradi.
            ...
            # create_expense_journal_entry() faqat cash hisobi kompaniya
            # valyutasidan farq qilganda kurs so'raydi.
...
@frappe.whitelist()
def is_company_restricted_user(user=None):
    """True -- agar user User Permission orqali kompaniyaga cheklangan bo'lsa.
    kassa.js buni "Центр затрат" maydonini yashirish uchun ishlatadi."""
